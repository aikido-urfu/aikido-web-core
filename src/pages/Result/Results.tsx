    ...
    <div>
      ...
      <span className="bg-white flex flex-col items pb-12  m-[30px] min-h-[900px]">
          ...
          <div className=' py-[10px]'>
            {selectedVote?.questions.map(x => (<div 
            style={{
            borderBottom: '1px solid #DADADA',
          }}>
              <h3 className='px-[10px] h-[50px] flex items-center'>{x.title}</h3>
              <div className='pl-[40px]' style={{
                ...
                {x.answers.map((answ, index )=> (
                  <>
                    <div className='flex gap-2 h-[80px] items-center'>
                      <span>{index + 1}.</span>
                      <span>{answ.text}</span>
                      <Progress className='max-w-[200px] ml-auto' percent={Math.round(answ.users.length / selectedVote.usersVoted.length * 100)} />
                    </div>
                  </>
                ))}
              </div>
